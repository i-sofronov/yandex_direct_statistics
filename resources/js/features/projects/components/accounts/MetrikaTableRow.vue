<template>
    <ModuleTableRow :key="id">
        <ModuleTableCell cell-width="65">
            <div :id="tooltipId" style="width: 100%; cursor: pointer" @click="toggle" class="module-opts" ref="menuButtonRef">
        ...
        <ModuleTableCell cell-width="125">
            <TableIndicator
                :indicator="total.pageviews"
                :dynamic="total.dynamic.pageviews"
            ></TableIndicator>
        </ModuleTableCell>

        <ModuleTableCell cell-width="100">
            <TableIndicator
                :indicator="total.visits"
                :dynamic="total.dynamic.visits"
            ></TableIndicator>
        </ModuleTableCell>
        ...
        <ModuleTableCell cell-width="125">
            <TableIndicator
                ...
                :dynamic="total.dynamic.users"
            ></TableIndicator>
        </ModuleTableCell>

        <ModuleTableCell cell-width="150">
            <TableIndicator
                :indicator="total.avg_visit_duration_seconds.toFixed(2)"
                :dynamic="total.dynamic.avg_visit_duration_seconds.toFixed(2)"
            ></TableIndicator>
        </ModuleTableCell>

        <ModuleTableCell cell-width="150">
            <TableIndicator
                :indicator="total.page_depth.toFixed(2)"
                :dynamic="total.dynamic.page_depth.toFixed(2)"
            ></TableIndicator>
        </ModuleTableCell>

        <ModuleTableCell cell-width="150">
            <TableIndicator
                :indicator="total.bounce_rate.toFixed(2)"
                :dynamic="total.dynamic.bounce_rate.toFixed(2)"
            ></TableIndicator>
        </ModuleTableCell>
...
import { YandexMetrikaTotalInterface } from '@/features/projects/models/Account.model';
...
const props = defineProps({
    ...
    total: {
        type: Object as PropType<YandexMetrikaTotalInterface>,
        required: true
    },
});

const tooltipId = computed(() => {
    return 'metrika-table-row-tooltip-' + props.id
})
